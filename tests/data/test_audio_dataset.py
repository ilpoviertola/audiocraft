    ...
    def test_save_audio_meta(self):
        audio_meta = [
            AudioMeta(
                ...
                None,
                None,
                PathInZip("/foo/bar.zip:/relative/file1.json"),
            ),
            AudioMeta(
                "mypath2",
                2.0,
                ...
                None,
                None,
                PathInZip("/foo/bar.zip:/relative/file2.json"),
            ),
        ]
        empty_audio_meta = []
...
class TestAudioDataset(TempDirMixin):
    ...
    def _create_audio_files(
        ...
        root_name: str,
        num_examples: int,
        durations: tp.Union[float, tp.Tuple[float, float]] = (0.1, 1.0),
    ...
    def _create_audio_dataset(
        ...
        root_dir = self._create_audio_files(
            root_name, total_num_examples, durations, sample_rate, channels
        )
        dataset = AudioDataset.from_path(
            root_dir,
            minimal_meta=True,
    ...
    def test_dataset_with_meta_collate_fn(self, segment_duration):
        ...
        dataloader = DataLoader(
            dataset, batch_size=batch_size, collate_fn=dataset.collater, num_workers=0
        )
        for idx, batch in enumerate(dataloader):
            wav, infos = batch
    ...
    @pytest.mark.parametrize(
        "segment_duration,sample_on_weight,sample_on_duration,a_hist,b_hist,c_hist",
        [
    ...
    def test_sample_with_weight(
        ...
        def _get_histogram(dataset, repetitions=20_000):
            counts = {file_meta.path: 0.0 for file_meta in meta}
        ...
        dataset = AudioDataset(
            meta,
            segment_duration=segment_duration,
            sample_on_weight=sample_on_weight,
    ...
    def test_meta_duration_filter_long(self):
        ...
        dataset = AudioDataset(
            meta, segment_duration=None, min_segment_ratio=1, max_audio_duration=7
        )
