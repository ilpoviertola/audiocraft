...
class TestSEANetModel:
    ...
    def _check_encoder_blocks_norm(
        self, encoder: SEANetEncoder, n_disable_blocks: int, norm: str
    ):
        n_blocks = 0
        for layer in encoder.model:
            if isinstance(layer, StreamableConv1d):
                n_blocks += 1
                assert (
                    layer.conv.norm_type == "none"
                    if n_blocks <= n_disable_blocks
                    else norm
                )
                ...
                for resnet_layer in layer.block:
                    if isinstance(resnet_layer, StreamableConv1d):
                        ...
                        assert (
                            resnet_layer.conv.norm_type == "none"
                            if (n_blocks + 1) <= n_disable_blocks
                            else norm
                        )
    ...
    def _check_decoder_blocks_norm(
        self, decoder: SEANetDecoder, n_disable_blocks: int, norm: str
    ):
        n_blocks = 0
        for layer in decoder.model:
            if isinstance(layer, StreamableConv1d):
                ...
                assert (
                    layer.conv.norm_type == "none"
                    if (decoder.n_blocks - n_blocks) < n_disable_blocks
                    else norm
                )
            elif isinstance(layer, StreamableConvTranspose1d):
                n_blocks += 1
                assert (
                    layer.convtr.norm_type == "none"
                    if (decoder.n_blocks - n_blocks) < n_disable_blocks
                    else norm
                )
            elif isinstance(layer, SEANetResnetBlock):
