...
def read_txt_files(path: tp.Union[str, Path]):
    with open(args.files_path) as f:
        ...
        lines = [
            line for line in lines if Path(line).suffix not in [".json", ".txt", ".csv"]
        ]
...
def process_dataset(
    args, n_shards: int, node_index: int, task_index: tp.Optional[int] = None
):
    ...
    if args.files_path is None:
        lines = [
            m.path
            for m in find_audio_files(
                args.root_path, resolve=False, progress=True, workers=8
            )
        ]
    else:
    ...
    for idx, line in tqdm.tqdm(enumerate(lines)):
        ...
        try:
            ...
            if out_channels > 2:
                print(
                    f"Mix has more than two channels: {out_channels}, enforcing 2 channels"
                )
                out_channels = 2
...
if __name__ == "__main__":
    ...
    if args.files_path is None:
        print(
            "Warning: --files_path not provided, not recommended when processing more than 10k files."
        )
    if args.debug:
        ...
        with executor.batch():
            for node_index in range(args.nodes):
                job = executor.submit(
                    process_dataset, args, n_shards=n_shards, node_index=node_index
                )
                jobs.append(job)
        for job in jobs:
